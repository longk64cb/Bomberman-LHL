...
import uet.oop.bomberman.entities.staticobject.Bomb.Explosion;
...
public abstract class Enemy extends Character {
    ...
    public void load_Anh(Direction direction,String Name_Enemy){
        ...
        if(Name_Enemy.equals("doll")){
            if(direction == Direction.LEFT || direction == Direction.DOWN){
                if(direction == Direction.LEFT) img = Sprite.doll_left1.getFxImage();
                setSprite(Sprite.movingSprite(Sprite.doll_left1, Sprite.doll_left2, Sprite.doll_left3, animate, 20));
            }
            else{
                if(direction == Direction.RIGHT) img = Sprite.doll_right1.getFxImage();
                setSprite(Sprite.movingSprite(Sprite.doll_right1, Sprite.doll_right2, Sprite.doll_right3, animate, 20));
            }
        }
        if(Name_Enemy.equals("minvo")){
            if(direction == Direction.LEFT || direction == Direction.DOWN){
                if(direction == Direction.LEFT) img = Sprite.minvo_left1.getFxImage();
                setSprite(Sprite.movingSprite(Sprite.minvo_left1, Sprite.minvo_left2, Sprite.minvo_left3, animate, 20));
            }
            else{
                if(direction == Direction.RIGHT) img = Sprite.minvo_right1.getFxImage();
                setSprite(Sprite.movingSprite(Sprite.minvo_right1, Sprite.minvo_right2, Sprite.minvo_right3, animate, 20));
            }
        }
        if(Name_Enemy.equals("Kondoria")){
            if(direction == Direction.LEFT || direction == Direction.DOWN){
                if(direction == Direction.LEFT) img = Sprite.minvo_left1.getFxImage();
                setSprite(Sprite.movingSprite(Sprite.kondoria_left1, Sprite.kondoria_left2, Sprite.kondoria_left3, animate, 20));
            }
            else{
                if(direction == Direction.RIGHT) img = Sprite.minvo_right1.getFxImage();
                setSprite(Sprite.movingSprite(Sprite.kondoria_right1, Sprite.kondoria_right2, Sprite.kondoria_right3, animate, 20));
            }
        }
    }
