...
//	public static Sprite level = new Sprite(DEFAULT_SIZE, 0, 0,
//			new SpriteSheet("bomberman-starter-starter-2/res/load_level/level_1.png", 400), 16, 16);
	...
	public static Sprite finalMovingSprite(Sprite x1, Sprite x2, Sprite x3, Sprite x4, int vanishTime, int maxTime) {
		if (vanishTime > maxTime / 4 * 3) {
			return x1;
		}
		if (vanishTime > maxTime / 4 * 2 && vanishTime < maxTime / 4 * 3) {
			return x2;
		}
		if (vanishTime > maxTime / 4 && vanishTime < maxTime / 4 * 2) {
			return x3;
		}
		return x4;
	}
	...
	public static Sprite finalMovingSprite(Sprite normal, Sprite x1, Sprite x2, int time, int maxTime) {
		if (time > maxTime / 3 * 2) {
			return normal;
		}
		if (time > maxTime / 3 && time < maxTime / 3 * 2) {
			return x1;
		}
		return x2;
	}
